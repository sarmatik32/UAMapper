...
async function startServer() {
  ...
  // Same-origin Visicom fragment proxy used by HD export.
  app.get("/api/visicom-fragment", async (req, res) => {
    ...
    if (!z || !lng || !lat || !key || !Number.isFinite(width) || !Number.isFinite(height) || width < 1 || height < 1 || width > 2048 || height > 2048) {
      return res.status(400).send("Invalid Visicom fragment parameters");
    }

    const upstreamUrl = `https://tms.visicom.ua/2.0.0/planet3/base/${encodeURIComponent(z)}/${encodeURIComponent(lng)},${encodeURIComponent(lat)}/${Math.round(width)}/${Math.round(height)}.png?lang=${lang}&key=${encodeURIComponent(key)}`;

    try {
      const upstream = await new Promise<{ status: number; contentType: string; body: Buffer }>((resolve, reject) => {
        https.get(upstreamUrl, { headers: { "User-Agent": "UAMapper/1.0", Accept: "image/png,image/*;q=0.9,*/*;q=0.8" } }, (response) => {
          const chunks: Buffer[] = [];
      ...
      console.warn("[Visicom Proxy Warning]:", error?.message || error);
      return res.status(502).send(`Visicom proxy error: ${error?.message || "upstream request failed"}`);
    }
  });
